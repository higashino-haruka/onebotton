...
HRESULT RendererInit(HWND hwnd)
{
	...
	hr = D3D11CreateDeviceAndSwapChain(NULL,
		...
		&g_FeatureLevel,    // 作成されたデバイスの機能レベルを受け取る変数へのポインタ
...
void RendererDrawStart()
{
	...
	//サンプラーをピクセルシェーダーに渡す
	g_pDeviceContext->PSSetSamplers(0, 1, &g_pSampler);  //描画の際にテクスチャの情報をシェーダに渡す
...
void RendererUninit()
{
	...
	SAFE_RELEASE(g_pConstantBuffer);  //定数バッファを解放
